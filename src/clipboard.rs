...
const NCCDESK_CLIPBOARD_OWNER_FORMAT: &'static str = "dyn.com.nccdesk.owner";
...
const SUPPORTED_FORMATS: &[ClipboardFormat] = &[
    ...
    ClipboardFormat::Special(NCCDESK_CLIPBOARD_OWNER_FORMAT),
];
...
fn do_update_clipboard_(mut to_update_data: Vec<ClipboardData>, side: ClipboardSide) {
    ...
    if let Some(ctx) = ctx.as_mut() {
        to_update_data.push(ClipboardData::Special((
            NCCDESK_CLIPBOARD_OWNER_FORMAT.to_owned(),
            side.get_owner_data(),
        )));
...
impl ClipboardContext {
    ...
    fn get_formats_filter(
        ...
        if !force {
            for c in data.iter() {
                if let ClipboardData::Special((s, d)) = c {
                    if s == NCCDESK_CLIPBOARD_OWNER_FORMAT && side.is_owner(d) {
                        return Ok(vec![]);
                    }
                }
            }
        ...
        Ok(data
            .into_iter()
            .filter(|c| match c {
                ClipboardData::Special((s, _)) => s != NCCDESK_CLIPBOARD_OWNER_FORMAT,
