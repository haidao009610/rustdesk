// https://github.com/nccdesk/nccdesk/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteNccDeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes NccDesk Test certificates and wrong key stores
//
...
BOOL DeleteNccDeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteNccDeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteNccDeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteNccDeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteNccDeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
